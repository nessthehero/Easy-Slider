...
(function($) {
	$.easySlider = function(el, options) {
				...
				if (base.options.preload && (!$.browser.msie && $.browser.version != "8.0")) {
					...
					switch (base.options.preloadMethod) {
						...
						case 'bg':
							...
							// Doesn't work great if the URI is an IP address, so I'm using a backup plan. Data attributes.
							//var bgSrc = base.first.attr('data-bgimg');
							var bgSrc = base.first.css('backgroundImage').toString().replace(/url\(/,'').replace(/\)/,'').replace(/"/g,'').toString();
							//console.log('it is now: ' + bgSrc);
							var bgImg = new Image();
		...
		base.getRequirements = function() {
			if (base.options.loadrequire() && !base.loaded) {
				base.loaded = true;
				base.options.loadrequire = function() { return base.loaded; }
				base.readyShow();
			} else {
				setTimeout(function() {
						base.getRequirements();
	...
	$.easySlider.defaultOptions = 
    {
		...
		preloadMethod: 'img',
		loadimage: '',
		loadrequire: function() { return true; },
		start: function() {},
		end: function() {},
		before: function() {},
		after: function() {}
